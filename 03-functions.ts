...
// Function Types
// Define the shape of a function using a function type.
...
// Define multiple signatures for a function to handle different types.
function triple(x: number): number;
function triple(x: string): string;
function triple(x: any): any {
    if (typeof x === "number") {
        return x * 3;
...
// This Typing
// Specify the type of 'this' in functions, useful for methods.
interface User {
    username: string;
    displayName(this: User): void;
}
